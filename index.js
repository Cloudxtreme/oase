 ...
 */
 ...
 function flat_map_sort_on_key(map) {
   let rc = flat_map_files(map, false);
   rc.files.sort((a, b) => {
     let k1 = a[0];
     let k2 = b[0];
     if (k1 > k2) {
   ...
   return new Map(rc.files);
 };

 function flat_map_files(map, exclude_hashed_files) {
   ...
   function reduce(_map) {
     for (var entry of _map) {
       ...
       if (entry[1].hmac && exclude_hashed_files) {
         continue;
       }
       rc.files.push(entry);
       rc.count++;
       rc.total_size += entry[1].size || 0;
     }
   }
   reduce(map);
   //sort on keys
 ...
 */
 ...
 function dir_map_to_object_json(map) {
   let rc = {};
   if (!(map instanceof Map)) {
     return undefined;
   }
   map.forEach((stat, key, mm) => {
     if (stat instanceof Map) { //subdir
       let sub_dir = dir_map_to_object_json(stat);
       if (sub_dir) {
         rc[key] = sub_dir;
       }
       return;
 ...
 function json_to_dir_map(json_text) {
   ...
   let root = new Map();
   json_obj_to_dir_map(json, root);
   return root;
 };

 function json_obj_to_dir_map(json_obj, map) {
   let keys = Object.keys(json_obj);
   keys.forEach((key) => {
     let entry = json_obj[key];
     // is a dir?
     if (entry["."]) {
       ...
       children.set(".", entry["."]);
       delete entry["."];
       json_obj_to_dir_map(entry, children);
       return;
     }
     map.set(key, json_obj[key]);
   });
 }
 ...
 function open_dictionary(chain_ctx) {
   ...
   fs.readFile(confFile, "utf8", (err, data) => {
     if (err && !is_resumable_io_err(err)) {
 ...
 function is_actual_equal_to_dictionary(actual, dict_entry) {
   ...
   if (dict_entry.mtime != actual.mtime.toJSON()) {
     return false;
   }
   return true;
 }
 ...
 function hash_files(chain_ctx) {
   ...
   // -- all ok
   const dir_map = storage.dir_map;
   var collection = flat_map_files(dir_map, true);
   storage.file_count_to_do = [collection.count, collection.total_size];
   storage.state = STATE_HASHING;
   // -- bootstrap base dirs to the count todo
   storage.file_count_done = [0, 0];
   storage.files_being_processed = new Map();
   //
   var all_files = collection.files;
 ...
 function save_dictionary(chain_ctx) {
   ...
   //
   fs.writeFile(confFile, JSON.stringify(json_obj), "utf8", (err) => {
     if (err) {
       storage.state = STATE_UNUSABLE;
       storage.alerts.push(err);
       console.log("dictionary un-usable.");
